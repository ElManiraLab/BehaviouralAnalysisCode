...
for i = 1:NumberFiles
    ...
    if isempty(isFIG) && isempty(SIEorSW) && ~isempty(TableOrVars) % If doesnt have it means is the Variables
        j = j + 1 ;
...
% EACH FISH IS A COLUMN. FOR TABLE VARIABLES THAT ARE STRUCTURES (MORE THAN 1 EPISODE)
...
DistanceFromTop = 3;
LateralDistance = 20;
NameDestin = 'IRENE_TIFS_COLUMNS.xlsx';
...
for i = 1:NumberFiles
    ...
    if  isempty(SIEorSW) && isempty(isFIG) && ~isempty(TableOrVars)
        ...
        if  j == 1 || WeekDouble(j)==WeekDouble(j-1)
            ...
            if FishDouble(j) == str2double(Fish) && j == 1
                ...
                FN = fieldnames(TableAnimal) ;

                for k = 1:length(fieldnames(TableAnimal))
                    matrixEpisode = table2array(TableAnimal.(string(FN(k))));
                    TA(1+EpiosdeNumber:size(matrixEpisode,1)+EpiosdeNumber,1:size(matrixEpisode,2)) = ...
                        matrixEpisode;

                    EpiosdeNumber = size(matrixEpisode,1) + EpiosdeNumber;
                    clear matrixEpisode
            ...
            elseif FishDouble(j) == str2double(Fish) && FishDouble(j)==FishDouble(j-1) &&...
                ...
                for k = 1:length(fieldnames(TableAnimal))
                    myfield1 = table2array(TableAnimal.(string(FN(k))));
                ...
                if exist('Animal','var')
                    ...
                    writematrix(Animal,fullfile(path.MainFolder,NameDestin),'Sheet',strcat('Condition',num2str(DateDouble(j-1))),'Range',strcat(Column,Row));
                    RowName=  num2str(1);

                    writecell({strcat(Week,'_','Fish',num2str(FishDouble(j-1)))},fullfile(path.MainFolder,NameDestin),'Sheet'...
                        ,strcat('Condition',num2str(DateDouble(j-1))),'Range',strcat(Column, RowName))

                    writecell(  TableAnimal.(string(FN(k))).Properties.VariableNames,fullfile(path.MainFolder,NameDestin),'Sheet',...
                        strcat('Condition',num2str(DateDouble(j-1))),'Range',strcat(Column, RowParam ))
    ...
    %% EACH TABLE IS A COLUMN. FOR TABLE VARIABLES THAT ARE STRUCTURES (MORE THAN 1 EPISODE)
        ...
        if  ~isempty(TableOrVars)
            ...
            for k=1:length(fieldnames(EPISODE_X))
                ...
                myfield1 = char(temp1(k))
                 if length(EPISODE_X.(myfield1))==1
                 else 
